...
fun CheckoutScreen(
    ...
    LaunchedEffect(Unit){
        ...
        viewModel.checkoutEffect.collectLatest {
            when(it){
                is CheckoutEffect.NavigateToPaymentStatus -> navigateToPaymentCompleted(it.isCompleted, it.errorMessage)
                is CheckoutEffect.OpenExternalLink -> openWebBrowser(it.url)
                is CheckoutEffect.ShowError -> messageBarState.addError(it.message)
            }
        }
    }

    Box(modifier = modifier.fillMaxSize()) {
        if (screenState is RequestState.Loading) {
